...
# Global variable to hold the last triggered signal if needed
PAST_SIGNALS = "HOLD"
...
def main():
    ...
    # Initialize PositionManager (it uses the Binance Client from TradeExecutor)
    ...
    if args.mode == "backtest":
        ...
        # Debug: Print first few rows for 15m and 1h data after indicator calculation.
        ...
        trades = []  # To store trade summaries

        global PAST_SIGNALS  # To track signal changes
        PAST_SIGNALS = "HOLD"

        # Backtest loop: iterate over each 15m candle
        for idx, row in df_15m.iterrows():
            ...
            # Extract the most recent 1h candle for current simulation time.
            ...
            # Create a live window of data (all intervals up to ts)
            live_data = {}
            ...
            # Generate signals from the current window.
            ...
            # --- Signal change logic ---
            # Compute candidate signal based on MACD values from both intervals.
            ...
            # Update risk management for the current candle.
            ...
            # Check if a trade closed on this candle; capture its PnL info.
        ...
        # Live mode branch (similar structure; not modified here)
        polling_interval = 15 * 60  # 15 minutes in seconds
        while True:
            current_price = market_data.fetch_live_price()
            if current_price is None:
                logging.error("Live price fetch returned None. Waiting...")
                time.sleep(60)
                continue
            logging.info(f"Live price for {args.pair}: {current_price}")
            ...
            # (Live branch similar trade execution & logging as backtest)
            # ... [live code would be here] ...
